...
INSERT INTO `point_rules` (`code`, `name`, `category`, `points`, `daily_limit`, `enabled`, `description`) VALUES
  ('training_deal', 'AI对练-成交', 'training', 20, 0, 1, '一次销售对练复盘得分 ≥80 时入账'),
  ('training_intent', 'AI对练-意向客户', 'training', 10, 0, 1, '一次销售对练复盘得分 ≥60 且 <80 时入账'),
  ('training_other', 'AI对练-其他完成', 'training', 3, 5, 1, '一次销售对练完成但得分 <60，每日最多 5 次入账'),
...
-- 历史遗留：连续打卡奖励规则已下线，清理已存在的旧规则。
DELETE FROM `point_rules` WHERE `code` IN ('reading_streak_7', 'reading_streak_30');
