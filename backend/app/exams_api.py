...
async def _notify_exam_assigned_in_session(session, exam_id: int) -> None:
...
async def batch_create_exams(
    ...
    await db.commit()
    for exam_id_for_notify in notify_exam_ids:
        await safe_dispatch(
            lambda session, eid=exam_id_for_notify: _notify_exam_assigned_in_session(session, eid),
            event="exam_assigned",
            business_id=exam_id_for_notify,
        )
    return out
...
async def submit_admin_review(
    ...
    await db.commit()
    await safe_dispatch(
        lambda session: _notify_exam_reviewed_in_session(session, attempt_id_for_notify),
        event="exam_reviewed",
        business_id=attempt_id_for_notify,
    )
...
def build_user_router(*, settings: Settings, rule_loader: RuleLoader) -> APIRouter:
    ...
    async def finish_exam(
        ...
        await db.commit()
        await safe_dispatch(
            lambda session: _notify_exam_pending_review_in_session(session, attempt_id_for_notify),
            event="exam_pending_review",
            business_id=attempt_id_for_notify,
        )
